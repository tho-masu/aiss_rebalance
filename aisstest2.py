...
def cossim(vec0, vec1):
    ...
    sim = iprod(vec0, vec1) / (l2norm(vec0)*l2norm(vec1))
    return (math.pi/2 - math.acos(sim)) / (math.pi/2)
...
f = ((1, 0, 0, 0),  # ファンド0
     ...
     (0, 0, 0, 1),  # ファンド3
     (0, 0.5, 0, 0.5),  # ファンド4
     (0.3, 0.2, 0.3, 0.2),  # ファンド5
     (0.1, 0.1, 0.4, 0.4))  # ファンド6

t = (0.4, 0.1, 0.2, 0.3)

fundarr = [0, 20, 0, 70, 50, 0, 40]
...
print("----------------------START----------------------")
print("資産クラス配分:", A)
print("資産クラス配分（割合）:", l1(A))
print("理想資産クラス配分（割合）:", l1(t), "\n")
print("保有ファンド:", fundarr, "\n")
print("購入残り:", beta)
print("資産クラス購入額一覧:", bought)
print("売却残り:", delta)
print("ファンド売却額一覧:", sold, "\n")
print("類似度:", cossim(A, t), "\n")
